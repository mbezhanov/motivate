...
const importQuotes = (shouldShowPrompt, onImportFinished, onImportFailed) => {
    ...
    return FileSystem
      ...
      .then(contents => new Promise((resolve, reject) => {
        ...
        if (!shouldShowPrompt) {
          ...
          return _doCsvImport(contents, IMPORT_MODE_APPEND, onImportFinished, onImportFailed);
        }
        Alert.alert('Import Settings', 'Would you like to append quotes to your existing collection or overwrite it completely?', [
          { text: 'Overwrite', onPress: () => _doCsvImport(contents, IMPORT_MODE_OVERWRITE, onImportFinished, onImportFailed) },
          { text: 'Append', onPress: () => _doCsvImport(contents, IMPORT_MODE_APPEND, onImportFinished, onImportFailed) }
        ]);
      }));
    });
};

const _doCsvImport = (contents, importMode, onImportFinished, onImportFailed) => {
